...
// Класс Boss, который наследует от Enemy
class boss : public Enemy {
private:
    std::string spetialAbility; // Свойство Специальная Способность

public:
    boss(const std::string& n, int h, const std::string& t, const std::string& sa)  //Переопределение конструктора
        : Enemy(n, h, t), spetialAbility(sa) {} 
    void displayInfo() const {  //Переопределение вывода информации
        Enemy::displayInfo();
